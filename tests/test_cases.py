...
def run_test_case(name, environment, job, feedback, student_profile=None, conflict_method='combined'):
    ...
    recommendation = advisor.advise(environment=environment,job=job,feedback=feedback,student_profile=student_profile )    
    if recommendation:
        print(f"\n Test result '{name}':")
...
def test_conflict_scenarios():
    ...
    run_test_case( name="Conflict Scenario 1: Programming student", environment="computer programs", job="writing",feedback="required" )
    # Scenario 2: Multiple rule conflict
    run_test_case( name="Conflict Scenario 2: Advanced researcher",environment="computer programs", job="evaluating", feedback="required", student_profile={'level': 'advanced', 'time_flexibility': 'high'} )
    # Scenario 3: Conflict with multiple media
    run_test_case(name="Conflict Scenario 3: Instructional designer",environment="diagrams",job="evaluating", feedback="required",student_profile={'learning_style': 'visual'})
    # Scenario 4: No conflict
    run_test_case(name="Simple Scenario: Maintenance worker", environment="machines",job="repairing",feedback="required"  )
def test_different_resolution_methods():
    ...
    scenario = {'environment': 'computer programs','job': 'writing','feedback': 'required','student_profile': {'level': 'intermediate'} }
